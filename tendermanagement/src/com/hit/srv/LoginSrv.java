        ...
        if(user.toLowerCase().equals("login as admin")){
            ...
            try {
                ...
                if(rs.next()){ // Vendor Login Successful
                    ...
                    // Redirect to OTP Generation Servlet
                    RequestDispatcher rd = request.getRequestDispatcher("OtpVerificationServlet");
                    rd.forward(request, response);
                    ...
                    if(rs1.next()){
                        ...
                        session.setAttribute("vendordata", vendor);  // We need the user data for the whole session
